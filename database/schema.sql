...
create table if not exists public.stolen_artworks (
  ...
  city_of_theft text,
  institution text,
  theft_year integer check (theft_year between 1800 and 2100),
...
-- Read-only public policy for the dashboard API. Inserts/updates are performed only by the server-side Netlify function with the service-role key.
...
drop policy if exists "Allow public read of ETL runs" on public.etl_runs;
create policy "Allow public read of ETL runs" on public.etl_runs
for select using (true);
